...
create memory table T_BUG_REPORT ( BUR_ID_C varchar(36) not null, BUR_IDUSER_C varchar(36) not null, BUR_NAMEUSER_C varchar(50) not null, BUR_DESCRIPTION_C longvarchar not null, BUR_CREATEDATE_D datetime not null, BUR_RESOLVED_B bit not null, primary key (BUR_ID_C));
alter table T_BUG_REPORT add constraint FK_BUR_IDUSER_C foreign key (BUR_IDUSER_C) references T_USER (USE_ID_C) on delete restrict on update restrict;
-- custom feed
create cached table T_CUSTOMFEED (CFD_ID_C varchar(36) not null, CFD_CUSTOMUSERID_C varchar(36) not null, CFD_TITLE_C varchar(100) not null, CFD_DESCRIPTION_C varchar(4000) not null, primary key (CFD_ID_C) );
alter table T_CUSTOMFEED add constraint FK_CFD_CUSTOMUSERID_C foreign key (CFD_CUSTOMUSERID_C) references T_USER (USE_ID_C) on delete cascade on update cascade;
-- custom article
create cached table T_CUSTOMARTICLE ( CFA_ORIG_ID varchar(36) not null, CFA_ID varchar(36) not null, CFA_IDCUSTOMFEED_C varchar(36) not null, CFA_TITLE_C varchar(4000) not null, CFA_DESCRIPTION_C longvarchar, CFA_URL_C varchar(2000), CFA_GUID_C varchar(2000), CFA_CREATOR_C varchar(200), CFA_COMMENTURL_C varchar(2000), CFA_COMMENTCOUNT_N int, CFA_ENCLOSUREURL_C varchar(2000), CFA_ENCLOSURELENGTH_N int, CFA_ENCLOSURETYPE_C varchar(2000), CFA_PUBLICATIONDATE_D datetime, CFA_CREATEDATE_D datetime not null, primary key (CFA_ORIG_ID) );
-- create cached table T_CUSTOMARTICLE (CFA_ID varchar(36) not null, CFA_IDCUSTOMFEED_C varchar(36) not null, CFA_TITLE_C varchar(4000) not null, CFA_DESCRIPTION_C longvarchar, primary key (CFA_ID) );
alter table T_CUSTOMARTICLE add constraint FK_CFA_IDCUSTOMFEED_C foreign key (CFA_IDCUSTOMFEED_C) references T_CUSTOMFEED (CFD_ID_C) on delete cascade on update cascade;
-- custom feed subscription
create cached table T_CUSTOM_FEED_SUBSCRIPTION ( CFS_ID_C varchar(36) not null, CFS_IDUSER_C varchar(36) not null, CFS_IDFEED_C varchar(36) not null, CFS_TITLE_C varchar(100));
alter table T_CUSTOM_FEED_SUBSCRIPTION add constraint FK_CFSUSER_IDCUSTOMFEED_C foreign key (CFS_IDUSER_C) references T_USER (USE_ID_C) on delete cascade on update cascade;
alter table T_CUSTOM_FEED_SUBSCRIPTION add constraint FK_CFSID_USER foreign key (CFS_IDFEED_C) references T_CUSTOMFEED (CFD_ID_C) on delete restrict on update restrict;    
