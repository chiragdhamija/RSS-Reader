...
public class UserDao extends BaseDao<UserDto, UserCriteria> {
   ...
   public String create(User user) throws Exception {
    // System.out.println("kasdjfhkajsdhfkasjdhfkajsdhfkajsdhfkjashdkfjhas");
    // System.out.println(user);

    // Create the user UUID
    user.setId(UUID.randomUUID().toString());

    // Checks for user uniqueness (both username and email)
    EntityManager em = ThreadLocalContext.get().getEntityManager();
    Query q = em.createQuery(
        "SELECT u FROM User u WHERE (u.username = :username OR u.email = :email) AND u.deleteDate IS NULL"
    )
    ...
    List<?> existingUsers = q.getResultList();
