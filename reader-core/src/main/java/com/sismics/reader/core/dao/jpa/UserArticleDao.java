...
public class UserArticleDao extends BaseDao<UserArticleDto, UserArticleCriteria> {
    ...
    protected QueryParam getQueryParam(UserArticleCriteria criteria, FilterCriteria filterCriteria) {
        ...
        //for f2 
        boolean hasFeedIds = criteria.getFeedIds() != null && !criteria.getFeedIds().isEmpty();
        if (criteria.isSubscribed() && !hasFeedIds) {
        ...
        //for f2
        boolean hasCategoryIds = criteria.getCategoryIds() != null && !criteria.getCategoryIds().isEmpty();

List<String> feedIdsFromSubscriptions = new ArrayList<>();

if (hasFeedIds) {
    for (String subscriptionId : criteria.getFeedIds()) {
        feedIdsFromSubscriptions.add(getFeedIdFromSubscription(subscriptionId));
    }
}

if (hasFeedIds && hasCategoryIds) {
    // Both feed and category IDs present → OR condition
    criteriaList.add("(a.ART_IDFEED_C in (:feedIds) or fs.FES_IDCATEGORY_C in (:categoryIds))");
    parameterMap.put("feedIds", feedIdsFromSubscriptions);
    parameterMap.put("categoryIds", criteria.getCategoryIds());
} else {
    if (hasFeedIds) {
        criteriaList.add("a.ART_IDFEED_C in (:feedIds)");
        parameterMap.put("feedIds", feedIdsFromSubscriptions);
    }

    if (hasCategoryIds) {
        criteriaList.add("fs.FES_IDCATEGORY_C in (:categoryIds)");
        parameterMap.put("categoryIds", criteria.getCategoryIds());
    }
}
...
// end for f2
    ...
    //for f2
    private static final String FEED_ID_FROM_SUBSCRIPTION_QUERY = 
    "SELECT f.FED_ID_C " +
    "  FROM T_FEED_SUBSCRIPTION fs " +
    "  JOIN T_FEED f ON fs.FES_IDFEED_C = f.FED_ID_C " +
    " WHERE fs.FES_ID_C = :subscriptionId " +
    "   AND fs.FES_DELETEDATE_D IS NULL " +
    "   AND f.FED_DELETEDATE_D IS NULL";

private String getFeedIdFromSubscription(String subscriptionId) {
    EntityManager em = ThreadLocalContext.get().getEntityManager();
    Query query = em.createNativeQuery(FEED_ID_FROM_SUBSCRIPTION_QUERY);
    query.setParameter("subscriptionId", subscriptionId);

    return fetchSingleStringResult(query);
}

/**
 * Helper method to fetch single String result from a query safely.
 */
private String fetchSingleStringResult(Query query) {
    try {
