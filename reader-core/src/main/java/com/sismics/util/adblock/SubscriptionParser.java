...
public class SubscriptionParser extends DefaultHandler {
    ...
    public void startElement(String uri, String localName, String qName, Attributes attributes) throws SAXException {
        if (qName.equalsIgnoreCase(SUBSCRIPTION)) {
            ...
            currentSubscription.setTitle(attributes.getValue(TITLE));
            currentSubscription.setSpecialization(attributes.getValue(SPECIALIZATION));
            currentSubscription.setUrl(attributes.getValue(URL));
            currentSubscription.setHomepage(attributes.getValue(HOMEPAGE));
            String prefix = attributes.getValue(PREFIXES);
            if (prefix != null) {
                String[] prefixes = prefix.split(",");
                currentSubscription.setPrefixes(prefixes);
            }
            currentSubscription.setAuthor(attributes.getValue(AUTHOR));
        }
