...
r.feed.subscriptionTotalCounts = {}; 
...
r.feed.init = function() {
  ...
  $.History.bind('/feed/', function(state, target) {
    ...
    if (target == 'all') {
      ...
      // Configuring context for /trending
      r.feed.context.url = r.util.url.trending;
      r.feed.context.unread = false;
      $('#trending-feed-button').addClass('active');

      // Specific toolbar actions for trending
      r.feed.cache.toolbar.find('> .trending').removeClass('hidden');
      ...
      r.feed.cache.toolbar.find('> .search').removeClass('hidden');
        ...
        // ------------- [PATCH] New selection route -------------
        else if (target.substring(0, 10) == 'selection/') {
          // Example: #/feed/selection/mixed/subscriptionId:abc,categoryId:def
          var selection = target.substring(10); // remove "selection/" prefix
          var parts = selection.split('/');
          var selectionType = parts[0]; // e.g. "mixed"
          ...
          // Use the new resource endpoint for 'mixed'
          if (selectionType === 'mixed') {
            r.feed.context.url = '../api/mixed/selection?ids=' + ids.join(',');
          } else {
            // Fallback if other selection types exist
            r.feed.context.url = '../api/feed/selection?type=' + selectionType + '&ids=' + ids.join(',');
          }
    
          // In some conditions, re-update the subscription tree
          if (target.indexOf('selection/') === -1) {
  ...
  r.feed.activeXhr = r.util.ajax({
    ...
    data: data,
      ...
      var nbArticles = $(data.articles).length;
       ...
       // [ADDITION] If we’re loading a specific feed for the first time, store total just once
  if (!next && r.feed.context.subscriptionId) {
    var sid = r.feed.context.subscriptionId;
    if (typeof r.feed.subscriptionTotalCounts[sid] === 'undefined') {

      console.log(nbArticles);
      //If the backend does NOT provide total_count, at least store
      // how many articles we see on this first load:
      r.feed.subscriptionTotalCounts[sid] = nbArticles;
    }
  }
      ...
      if(r.feed.context.url == r.util.url.trending){
        r.feed.context.fullyLoaded = true;
      }
      // Building articles
