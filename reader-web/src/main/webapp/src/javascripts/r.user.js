    ...
    $('#signup-form').submit(function(event) {
      ...
      r.util.ajax({
        ...
        data: {
          ...
          $().toastmessage('showSuccessToast', $.t('Signup successful! Please log in.'));

          // Clear the signup form
          $('#signup-form')[0].reset();
          $('#toggle-auth-mode').click();
          
          // Pre-populate the username field in the login form
          $('#login-username-input').val(username);
        },
        fail: function(data) {
          if(data.responseJSON.type=="AlreadyExistingEmail"){
          $().toastmessage('showErrorToast', $.t('The email is already in use'));
         }
         else if(data.responseJSON.type=="AlreadyExistingUsername"){
          $().toastmessage('showErrorToast', $.t('The username is already in use'));
         }
          else{
            $().toastmessage('showErrorToast', $.t('Error signing up'));
        }
      }
      });
    });
...
r.user.setNarrowArticle = function(narrow) {
  ...
  console.log("l397 narrow = ", narrow);
  // Calling API
  r.util.ajax({
    url: r.util.url.user_update,
    data: {
