...
r.article.build = function(article, classes) {
  ...
  var trending=false
  if ('starred_count' in article) {
    trending=true
  }

  console.log("BUILDING");
  console.log(article, trending);
  ...
  if(trending){
    item.find('.header .feed-item-star').remove();
    item.find('.footer .feed-item-star').remove();
    item.find('.feed-item-subscription').remove();
    item.find('.feed-item-unread').remove();
    item.find('.footer').append('<div>STARRED BY ' + article.starred_count + ' USERS</div>');
  }
